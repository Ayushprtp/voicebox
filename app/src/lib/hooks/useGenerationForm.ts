...
const generationSchema = z.object({
  ...
  modelSize: z.enum(['1.7B', '0.6B', '1B', '3B', 'tts-1', 'tts-1-hd', 'omnivoice']).optional(),
...
export function useGenerationForm(options: UseGenerationFormOptions = {}) {
  ...
  async function handleSubmit(
    ...
    try {
      ...
      const remoteModel = data.modelSize || 'tts-1';
      const modelName =
        engine === 'luxtts'
          ? 'luxtts'
          : engine === 'chatterbox'
              ...
              : engine === 'tada'
                ...
                : engine === 'kokoro'
                  ...
                  : engine === 'remote_tts_clone'
                    ? 'remote-clone'
                    : engine === 'remote_tts'
                      ? remoteModel === 'omnivoice'
                        ? 'remote-omnivoice'
                        : remoteModel === 'tts-1-hd'
                          ? 'remote-tts-1-hd'
                          : 'remote-tts-1'
      ...
      const hasModelSizes =
        engine === 'qwen' || engine === 'qwen_custom_voice' || engine === 'tada' || engine === 'remote_tts';
